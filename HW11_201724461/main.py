...
def readPlan():
    ...
    parameters = {'pType':0, 'pFileName':'', 'aType':0, 'delta':0,
                   ...
                   'limitEval':0, 'numExp':0,
                    'popSize':0, 'uXp':0, 'mrF':0, 'XR':0, 'mR':0, 'resolution': 0}
...
def createProblem(parameters): ###
    ...
    pDic = {1: "numeric(parameters)",
            2: "tsp(parameters)"}
    p = eval(pDic[parameters['pType']])
    return p
...
def createOptimizer(parameters): ###
    ...
    algDic = { 1: "steepestAscent(parameters)",
               2: "firstChoice(parameters)",
               3: "stochastic(parameters)",
               4: "gradientDescent(parameters)",
               5: "simulatedAnnealing(parameters)",
               6: "GA(parameters)"}
    alg = eval(algDic[parameters['aType']])
    return alg
